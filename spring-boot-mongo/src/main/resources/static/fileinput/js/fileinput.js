...
(function (factory) {
  ...
  if (!$.fn.fileinputBsVersion) {
    $.fn.fileinputBsVersion = (window.bootstrap && window.bootstrap.Alert
  ...
  $h = {
    ...
    INIT_FLAG: 'init-',
    SCRIPT_SRC: document && document.currentScript && document.currentScript.src
        || null,
    ...
    getZoomPlaceholder: function () { // used to prevent 404 errors in URL parsing
      ...
      return srcPath.substring(0, srcPath.lastIndexOf("/") + 1)
          + 'img/loading.gif' + zoomVar;
    },
    isBs: function (ver) {
    ...
    getFileRelativePath: function (file) {
      ...
      return String(file.newPath || file.relativePath || file.webkitRelativePath
    ...
    getFileId: function (file, generateFileId) {
      ...
      return (file.size + '_' + encodeURIComponent(relativePath).replace(/%/g,
          '_'));
    },
    ...
    canOrientImage: function ($el) {
      var $img = $(document.createElement('img')).css(
              {width: '1px', height: '1px'}).insertAfter($el),
    ...
    uniqId: function () {
      return (new Date().getTime() + Math.floor(
          Math.random() * Math.pow(10, 15))).toString(36);
    },
    cspBuffer: {
      ...
      stash: function (htmlString) {
        ...
        $el.find('[style]').each(function (key, elem) {
          var $elem = $(elem), styleDeclaration = $elem[0].style,
              id = $h.uniqId(), styles = {};
        ...
        var values = Object.values ? Object.values(outerDom) : Object.keys(
            outerDom).map(function (itm) {
          return outerDom[itm];
        });
      ...
      apply: function (domElement) {
        ...
        $el.find('[' + self.CSP_ATTRIB + ']').each(function (key, elem) {
          var $elem = $(elem), id = $elem.attr(self.CSP_ATTRIB),
    ...
    whenAll: function (array) {
      var s = [].slice,
          resolveValues = arguments.length === 1 && $h.isArray(array) ? array
              : s.call(arguments),
          deferred = $.Deferred(), i, failed = 0, value,
          length = resolveValues.length,
          remaining = length, rejectContexts, rejectValues, resolveContexts,
          updateFunc;
      ...
      updateFunc = function (index, contexts, values) {
        return function () {
          ...
          deferred.notifyWith(contexts[index] = this,
              values[index] = s.call(arguments));
          if (!(--remaining)) {
            deferred[(!failed ? 'resolve' : 'reject') + 'With'](contexts,
                values);
          }
        };
      };
  ...
  FileInput.prototype = {
      ...
      var self = this, tm = self.taskManager = {
        ...
        TasksPool: function (id) {
          ...
          tp.run = function (maxThreads) {
            ...
            // if run all at once
            if (!maxThreads) {
              var tasksDeferredList = $h.getObjectKeys(tp.tasks).map(
                  function (key) {
    ...
    _init: function (options, refreshMode) {
      ...
      if (self.maxTotalFileCount > 0 && self.maxTotalFileCount
          < self.maxFileCount) {
      ...
      self.fileManager = {
        ...
        getUploadStats: function (id, loaded, total) {
          var fm = self.fileManager,
              started = id ? fm.stats[id] && fm.stats[id].started || $h.now()
        ...
        isPending: function (id) {
          return $.inArray(id, self.fileManager.filesProcessed) === -1
    ...
    _initResumableUpload: function () {
      var self = this, opts = self.resumableUploadOptions,
          logs = $h.logMessages, rm, fm = self.fileManager;
      ...
      rm = self.resumableManager = {
        ...
        uploadResumable: function () {
          ...
          for (i = 0; i < total; i++) {
            rm.logs[i] = !!(rm.chunksProcessed[rm.id]
        ...
        processUpload: function () {
          var fd, f, id = rm.id, fnBefore, fnSuccess, fnError, fnComplete,
              outData;
          if (!opts.testUrl) {
            rm.uploadResumable();
            return;
        ...
        sendAjax: function (index, retry, deferrer) {
          var f, chunkSize = rm.chunkSize, id = rm.id, file = rm.file,
              $thumb = rm.$thumb,
              msgs = $h.logMessages, $btnDelete = rm.$btnDelete,
    ...
    _initTemplateDefaults: function () {
      var self = this, tMain1, tMain2, tPreview, tFileIcon, tClose, tCaption,
          ...
          tStats, tModalLabel, tDescClose,
          renderObject = function (type, mime) {
            return '<object class="kv-preview-data file-preview-' + type
                + '" title="{caption}" ' +
    ...
    _initPreviewCache: function () {
      ...
      self.previewCache = {
        ...
        get: function (i, isDisabled) {
          var ind = $h.INIT_FLAG + i, data = self.previewCache.data,
              ...
              parseTemplate = function (cat, dat, fname, ftype, ftr, ind,
                  fclass, t) {
                var fc = ' file-preview-initial ' + $h.SORT_CSS + (fclass ? ' '
                        + fclass : ''),
                    id = self.previewInitId + '-' + ind,
                    fileId = config && config.fileId || id;
                /** @namespace config.zoomData */
                return self._generatePreviewTemplate(cat, dat, fname, ftype, id,
          ...
          if (asData) {
            out = parseTemplate(cat, content, fname, ftype, ftr, ind,
                frameClass);
          } else {
            out = parseTemplate('generic', content, fname, ftype, ftr, ind,
                frameClass, cat)
    ...
    _parseError: function (operation, jqXHR, errorThrown, fileName) {
      ...
      var self = this, errMsg = $.trim(errorThrown + ''), textPre, errText,
          text;
    ...
    _listen: function () {
      var self = this, $el = self.$element, $form = self.$form,
          $cont = self.$container, fullScreenEv;
    ...
    _zoneDragEnter: function (e) {
      var self = this, dt = e.originalEvent.dataTransfer,
      ...
      if (self._raise('fileDragEnter',
          {'sourceEvent': e, 'files': dt.types.Files})) {
    ...
    _initSortable: function () {
      var self = this, $el = self.$preview, settings,
          selector = '.' + $h.SORT_CSS, $cont, $body = $('body'),
          $html = $('html'), rev = self.reversePreviewOrder,
          Sortable = window.Sortable, beginGrab, endGrab;
      ...
      settings = {
        ...
        onSort: function (e) {
          var oldIndex = e.oldIndex, newIndex = e.newIndex, i = 0,
              ...
              exceedsLast = len > 0 && newIndex >= len, $item = $(e.item),
              $first;
          if (exceedsLast) {
            newIndex = len - 1;
          }
    ...
    _initZoom: function () {
      ...
      if (!self.$modal || !self.$modal.length) {
        $dialog = $h.createElement($h.cspBuffer.stash(modalMain)).insertAfter(
            self.$container);
    ...
    _setZoomContent: function ($frame, navigate) {
      var self = this, $content, tmplt, body, title, $body, $dataEl, config,
          ...
          desc, $desc,
          $btnFull = $modal.find('.btn-kv-fullscreen'),
          $btnBord = $modal.find('.btn-kv-borderless'), cap, size,
          $btnTogh = $modal.find('.btn-kv-toggleheader'),
          dir = navigate === 'prev' ? 'Left' : 'Right',
          slideIn = 'slideIn' + dir, slideOut = 'slideOut' + dir, parsed,
          zoomData = $frame.data('zoom');
      if (zoomData) {
        zoomData = decodeURIComponent(zoomData);
        parsed = $zoomPreview.html().replace(self.zoomPlaceholder,
      ...
      self._handler($modal, 'keydown', function (e) {
        var key = e.which || e.keyCode, delay = self.processDelay + 1,
            $prev = $(this).find('.btn-kv-prev'),
            $next = $(this).find('.btn-kv-next'),
            vId = $(this).data('previewId'), vPrevKey, vNextKey;
        [vPrevKey, vNextKey] = self.rtl ? [39, 37] : [37, 39];
        $.each({prev: [$prev, vPrevKey], next: [$next, vNextKey]},
    ...
    _showModal: function ($frame) {
      ...
      $modal.data(
          {backdrop: false, fileinputPluginId: self.$element.attr('id')});
    ...
    _zoomSlideShow: function (dir, previewId) {
      var self = this,
          ...
          $targFrame, i, $thumb,
          thumbsData = self.getFrames().toArray(), thumbs = [],
          len = thumbsData.length, out;
      if (self.reversePreviewOrder) {
      ...
      for (i = 0; i < len; i++) {
        $thumb = $(thumbsData[i]);
        if ($thumb && $thumb.length && $thumb.find(
            '.kv-file-zoom:visible').length) {
    ...
    _resetCanvas: function () {
      ...
      if (self.imageCanvas && self.imageCanvasContext) {
        self.imageCanvasContext.clearRect(0, 0, self.imageCanvas.width,
    ...
    _resetPreview: function () {
      var self = this, out, cap, $div, hasSuc = self.showUploadedThumbs,
    ...
    _getPreviewTemplate: function (t) {
      var self = this, templates = self.previewTemplates,
    ...
    _updateInitialPreview: function () {
      ...
      if (self.showPreview) {
        $.each(u, function (key, setting) {
          self.previewCache.add(setting.content, setting.config, setting.tags,
              setting.append);
        });
        if (self.hasInitData) {
          self._initPreview();
    ...
    _uploadSingle: function (i, id, isBatch, deferrer) {
      var self = this, fm = self.fileManager, count = fm.count(),
          ...
          previewId = self._getThumbId(id), $thumb, chkComplete, $btnUpload,
          $btnDelete,
      ...
      fnBefore = function (jqXHR) {
        ...
        self._raise('filepreupload',
            [outData, previewId, i, self._getThumbFileId($thumb)]);
      ...
      fnSuccess = function (data, textStatus, jqXHR) {
        ...
        setTimeout(function () {
          if ($h.isEmpty(data) || $h.isEmpty(data.error)) {
            ...
            self._raise('fileuploaded',
                [outData, pid, i, self._getThumbFileId($thumb)]);
            if (!isBatch) {
      ...
      fnError = function (jqXHR, textStatus, errorThrown) {
        ...
        setTimeout(function () {
          ...
          $prog = self.showPreview && $thumb ? $thumb.find(
              '.file-thumb-progress') : '';
    ...
    _uploadBatch: function () {
      ...
      $.each(self.fileManager.stack, function (key, data) {
        if (!$h.isEmpty(data.file)) {
          self._setFileData(formdata, data.file,
              (data.nameFmt || ('untitled_' + ctr)), key);
        }
        ctr++;
      });
    ...
    _resetCaption: function () {
      ...
      setTimeout(function () {
        var cap = '', n, chk = self.previewCache.count(true),
            len = self.fileManager.count(), file,
    ...
    _getSize: function (bytes, sizes) {
      ...
      if (typeof func === 'function') {
        ...
        if (size === 0) {
          ...
          out = (size / Math.pow(self.bytesToKB, i)).toFixed(2) + ' '
              + sizes[i];
        }
      }
    ...
    _generatePreviewTemplate: function (
      ...
      var self = this, caption = self.slug(fname), prevContent,
          ...
          screenW = window.innerWidth || document.documentElement.clientWidth
              || document.body.clientWidth,
          config, title = caption, alt = caption, typeCss = 'type-default',
          getContent,
      ...
      getContent = function (vCat, vData, zoom, frameCss, vZoomData) {
        ...
        tokens = {
          ...
          'typeCss': typeCss,
          'footer': footer,
          'data': zoom && vZoomData ? self.zoomPlaceholder + '{zoomData}'
              : vData,
          'template': templ || cat,
    ...
    _previewFile: function (i, file, theFile, data, fileInfo) {
      ...
      self._validateImageOrientation($thumb.find('img'), file, previewId,
    ...
    _slugDefault: function (text) {
      ...
      return $h.isEmpty(text, true) ? '' : String(text).replace(
          /[\[\]\/\{}:;#%=\(\)\*\+\?\\\^\$\|<>&"']/g, '_');
    },
    ...
    _setFileDropZoneTitle: function () {
      ...
      if (!self.showPreview || $zone.length === 0 || self.fileManager.count()
          ...
          self.previewCache.count() > 0 || (!self.isAjaxUpload
              && self._hasFiles())) {
        return;
      }
    ...
    _validateImageOrientation: function ($img, file, previewId, fileId, caption,
        ftype, fsize, iData) {
      var self = this, exifObj = null, value,
          autoOrientImage = self.autoOrientImage, selector;
      ...
      self.setImageOrientation($img, $(selector), value,
    ...
    _setTabIndex: function (type, html) {
      ...
      return html.setTokens({
        tabIndexConfig: index === undefined || index === null ? ''
            : 'tabindex="' + index + '"'
      });
    },
    ...
    _initFileSelected: function () {
      var self = this, $el = self.$element, $body = $(document.body),
    ...
    readFiles: function (files) {
      ...
      var self = this, reader = self.reader,
          ...
          $status = self.$previewStatus, msgLoading = self.msgLoading,
          msgProgress = self.msgProgress,
          previewInitId = self.previewInitId, numFiles = files.length,
      ...
      readFile = function (i) {
        ...
        self.lock(true);
        var file = files[i], id = self._getFileId(file),
            previewId = previewInitId + '-' + id, fSizeKB, j, msg,
            ...
            func, knownTypes = 0, isImage, txtFlag,
            processFileLoaded = function () {
              ...
              if (self._raise('fileloaded', [file, previewId, id, i, reader])
                  && self.isAjaxUpload) {
                if (!isImageResized) {
                  fm.add(file);
                }
        ...
        if (self.isAjaxUpload && fm.exists(fileId) || self._getFrame(previewId,
            true).length) {
          var p2 = {
            id: previewId,
            index: i,
          ...
          if (self.isAjaxUpload) {
            ...
            self._raise('fileduplicateerror',
                [file, fileId, caption, fSizeKB, previewId, i]);
            readFile(i + 1);
            self._updateFileDetails(numFiles);
          } else {
            self._showError(msg, p2);
        ...
        if (!self._canPreview(file)) {
          canLoad = self.isAjaxUpload && self._raise('filebeforeload',
              [file, i, reader]);
          ...
          var hex, fileInfo, uint, byte, bytes = [], contents, mime,
              readImage = function () {
                ...
                newReader.onload = function (theFileNew) {
                  if (self.isAjaxUpload && !self._raise('filebeforeload',
                      [file, i, reader])) {
    ...
    resume: function () {
      var self = this, fm = self.fileManager, flag = false,
          rm = self.resumableManager;
      fm.bpsLog = [];
      fm.bps = 0;
      if (!self.enableResumableUpload) {
      ...
      self.paused = false;
      if (flag) {
        self._toggleResumableProgress(self.progressInfoTemplate,
            self.msgUploadBegin);
      }
      setTimeout(function () {
        rm.upload();
    ...
    paste: function (e) {
      var self = this, ev = e.originalEvent,
    ...
    disable: function () {
      ...
      $container.find('.kv-fileinput-caption').addClass(
    ...
    enable: function () {
      ...
      $container.removeClass('is-locked');
      $container.find('.kv-fileinput-caption').removeClass(
          'file-caption-disabled');
      $container.find(
          '.fileinput-remove, .fileinput-upload, .file-preview-frame button')
      .removeAttr('disabled');
    ...
    this.each(function () {
      ...
      if (!data) {
        ...
        opt = $.extend(true, {}, $.fn.fileinput.defaults, t,
            $.fn.fileinputLocales.en, l, options, self.data());
  ...
  $.fn.fileinput.defaults = {
    ...
    sanitizeZoomCache: function (content) {
      ...
      $container.find(
          'input,textarea,select,datalist,form,.file-thumbnail-footer').remove();
  ...
  $.fn.fileinputLocales.en = {
    ...
    bitRateUnits: ['B/s', 'KB/s', 'MB/s', 'GB/s', 'TB/s', 'PB/s', 'EB/s',
      'ZB/s', 'YB/s'],
    fileSingle: 'file',
    filePlural: 'files',
