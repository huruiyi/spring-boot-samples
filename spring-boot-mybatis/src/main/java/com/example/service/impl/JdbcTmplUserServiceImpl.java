...
public class JdbcTmplUserServiceImpl implements JdbcTmplUserService {
  ...
  public User getUser(Long id) {
    // 执行的SQL
    String sql = " select id, user_name,  note from t_user where id = ?";
    // 参数
  ...
  public List<User> findUsers(String userName, String note) {
    // 执行的SQL
    String sql = " select id, user_name,   note from t_user " + "where user_name like concat('%', ?, '%') "
  ...
  public User getUser2(Long id) {
    ...
    User result = this.jdbcTemplate.execute((Statement stmt) -> {
      ...
      // 执行的SQL
      String sql2 = " select id, user_name,  note from t_user"
          + " where id = " + id;
