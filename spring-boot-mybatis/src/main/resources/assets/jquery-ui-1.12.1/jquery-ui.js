      ...
      transition: function (other, distance) {
        ...
        each(space.props, function (key, prop) {
          ...
          if (startValue === null) {
            ...
            result[index] = clamp(
                (endValue - startValue) * distance + startValue, prop);
          }
        });
    ...
    $.extend($.effects, {
      ...
      createPlaceholder: function (element) {
        ...
        if (/^(static|relative)/.test(cssPosition)) {
          ...
          placeholder = $("<" + element[0].nodeName + ">").insertAfter(
              element).css({
        ...
        var args = _normalizeArguments.apply(this, arguments),
            ...
            prefilter = function (next) {
              ...
              if (defaultMode && (normalizedMode === "show" ||
                  (normalizedMode === defaultMode && normalizedMode
                      === "hide"))) {
                el.show();
              }
...
//>>demos: http://jqueryui.com/effect/
  ...
  var effectsEffectBlind = $.effects.define("blind", "hide",
      function (options, done) {
        ...
        if (placeholder) {
          placeholder.animate($.effects.clipToBox(animate), options.duration,
              options.easing);
        }
...
//>>demos: http://jqueryui.com/effect/
  ...
  var effectsEffectBounce = $.effects.define("bounce",
      function (options, done) {
        ...
        if (!distance) {
          distance = element[ref === "top" ? "outerHeight" : "outerWidth"]()
              / 3;
        }
...
//>>demos: http://jqueryui.com/effect/
  ...
  var effectsEffectSize = $.effects.define("size", function (options, done) {
    ...
    if (origin) {
      ...
      from.top = (original.outerHeight - from.outerHeight) * baseline.y
          + pos.top;
      from.left = (original.outerWidth - from.outerWidth) * baseline.x
          + pos.left;
    ...
    if (scale === "content" || scale === "both") {
      ...
      element.find("*[width]").each(function () {
        ...
        // Vertical props scaling
        if (factor.from.y !== factor.to.y) {
          childFrom = $.effects.setTransition(child, vProps, factor.from.y,
              childFrom);
          childTo = $.effects.setTransition(child, vProps, factor.to.y,
              childTo);
        }

        // Horizontal props scaling
        if (factor.from.x !== factor.to.x) {
          childFrom = $.effects.setTransition(child, hProps, factor.from.x,
              childFrom);
          childTo = $.effects.setTransition(child, hProps, factor.to.x,
              childTo);
        }
  ...
  var effectsEffectScale = $.effects.define("scale", function (options, done) {
    ...
    var el = $(this),
        ...
        percent = parseInt(options.percent, 10) ||
            (parseInt(options.percent, 10) === 0 ? 0 : (mode !== "effect" ? 0
                : 100)),

        newOptions = $.extend(true, {
          from: $.effects.scaledDimensions(el),
          to: $.effects.scaledDimensions(el, percent,
...
// Selectors
  $.ui.focusable = function (element, hasTabindex) {
    ...
    return focusableIfVisible && $(element).is(":visible") && visible(
        $(element));
  };
...
// with a string, so we need to find the proper form.
  var form = $.fn.form = function () {
    return typeof this[0].form === "string" ? this.closest("form") : $(
        this[0].form);
  };
...
//>>docs: http://api.jqueryui.com/tabbable-selector/
  ...
  var tabbable = $.extend($.expr[":"], {
    tabbable: function (element) {
      ...
      return (!hasTabindex || tabIndex >= 0) && $.ui.focusable(element,
          hasTabindex);
    }
  });
...
//>>css.theme: ../../themes/base/theme.css
  ...
  var widgetsAccordion = $.widget("ui.accordion", {
    ...
    _createIcons: function () {
      ...
      if (icons) {
        icon = $("<span>");
        this._addClass(icon, "ui-accordion-header-icon",
    ...
    _setOptionDisabled: function (value) {
      ...
      this._toggleClass(this.headers.add(this.headers.next()), null,
          "ui-state-disabled",
          !!value);
    },
    ...
    _processPanels: function () {
      ...
      this.panels = this.headers.next().filter(
    ...
    _selectmenuOptions: function (position) {
      ...
      return {
        ...
        classes: {
          ...
          first: {
            "ui-selectmenu-button-open": "ui-corner-" + (direction ? "top"
                : "tl"),
            "ui-selectmenu-button-closed": "ui-corner-" + (direction ? "top"
                : "left")
          },
          last: {
            "ui-selectmenu-button-open": direction ? "" : "ui-corner-tr",
            "ui-selectmenu-button-closed": "ui-corner-" + (direction ? "bottom"
                : "right")
          },
          only: {
      ...
      if (children.length) {
        ...
        $.each(["first", "last"], function (index, value) {
          ...
          if (instance && that["_" + instance.widgetName + "Options"]) {
            ...
            options.classes = that._resolveClassesValues(options.classes,
                instance);
  ...
  $.widget("ui.checkboxradio", [$.ui.formResetMixin, {
    ...
    _getCreateOptions: function () {
      ...
      this.label.contents().not(this.element[0]).each(function () {
        ...
        that.originalLabel += this.nodeType === 3 ? $(this).text()
            : this.outerHTML;
      });
    ...
    _toggleClasses: function () {
      ...
      if (this.options.icon && this.type === "checkbox") {
        this._toggleClass(this.icon, null, "ui-icon-check ui-state-checked",
            checked)
...
//>>css.theme: ../../themes/base/theme.css
  ...
  $.widget("ui.button", {
    ...
    _getCreateOptions: function () {
      ...
      this.originalLabel = this.isInput ? this.element.val()
          : this.element.html();
      if (this.originalLabel) {
        options.label = this.originalLabel;
      }
    ...
    _setOptions: function (options) {
      var newShowLabel = options.showLabel === undefined ?
              ...
              options.showLabel,
          newIcon = options.icon === undefined ? this.options.icon
              : options.icon;
    ...
    refresh: function () {
      ...
      var isDisabled = this.element.is("input, button") ?
          this.element[0].disabled : this.element.hasClass(
              "ui-button-disabled");
      ...
      return function () {
        if (!this.length || (this.length && this[0].tagName !== "INPUT") ||
            (this.length && this[0].tagName === "INPUT" && (
                this.attr("type") !== "checkbox" && this.attr("type")
                !== "radio"
            ))) {
          return orig.apply(this, arguments);
        }
    ...
    parseDate: function (format, value, settings) {
      ...
      var iFormat, dim, extra,
          ...
          shortYearCutoff = (typeof shortYearCutoffTemp !== "string"
              ? shortYearCutoffTemp :
              new Date().getFullYear() % 100 + parseInt(shortYearCutoffTemp,
                  10)),
  ...
  $.fn.datepicker = function (options) {
    ...
    return this.each(function () {
      typeof options === "string" ?
          $.datepicker["_" + options + "Datepicker"].apply($.datepicker,
    ...
    _getHandle: function (event) {
      return this.options.handle ?
          !!$(event.target).closest(
              this.element.find(this.options.handle)).length :
          true;
    },
      ...
      this._renderAxis = function (target) {
        ...
        for (i in this.handles) {
          ...
          if (this.handles[i].constructor === String) {
            this.handles[i] = this.element.children(
                this.handles[i]).first().show();
...
//>>css.theme: ../../themes/base/theme.css
  ...
  $.widget("ui.dialog", {
    ...
    _createWrapper: function () {
      ...
      this._addClass(this.uiDialog, "ui-dialog",
          "ui-widget ui-widget-content ui-front");
...
//>>css.theme: ../../themes/base/theme.css
  ...
  var widgetsSelectmenu = $.widget("ui.selectmenu", [$.ui.formResetMixin, {
    ...
    _drawButton: function () {
      ...
      this._addClass(icon, "ui-selectmenu-icon",
  ...
  var widgetsSortable = $.widget("ui.sortable", $.ui.mouse, {
    ...
    _intersectsWithPointer: function (item) {
      ...
      return this.floating ?
          ((horizontalDirection === "right" || verticalDirection === "down") ? 2
              : 1)
    ...
    _contactContainers: function (event) {
      ...
      if (this.containers.length === 1) {
        ...
        floating = innermostContainer.floating || this._isFloating(
            this.currentItem);
